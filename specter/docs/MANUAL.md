...
to distribute ten separate secrets by hand.
...
# Broker config, password file (11 accounts: 1 operator + 10 services), and
# the matching ACL file are all generated by the installer - hand-deriving
# ten per-service passwords and an ACL file in bash isn't practical to
...
`/etc/mosquitto/specter_passwd` (all 11 accounts), `/etc/mosquitto/specter_acl`
...
sudo systemctl enable --now specter-mqtt specter-coordinator specter-dashboard specter-thermal specter-trauma specter-ward
```
...
- ⚠️ **eventlet is deprecated upstream** — `dashboard_server.py` runs Flask-SocketIO with `async_mode="eventlet"`; eventlet itself is now maintained in bugfix-only mode (its own import emits `EventletDeprecationWarning`, visible throughout this project's test output). Not an immediate break, and no CVE-driven urgency as of this writing, but it should not be left running past its own maintainers' support window. Migrating to Flask-SocketIO's `gevent` async mode (closest drop-in for this workload) or `threading` (simpler, lower concurrency ceiling) is real work - it changes how the dashboard's WSGI server runs, not a one-line swap - and needs its own test pass against the Socket.IO push-based UI before shipping. No target date set; treat "eventlet gets no more upstream fixes" as the trigger to prioritize this, not a calendar date.
- ⚠️ **Test coverage is real but uneven** — CI now runs `pytest --cov` and publishes a per-file report as a GitHub Actions step summary (August 2026; an external review measured this independently and found it accurate), but there is no enforced threshold yet. Overall ~63%, and it is not evenly distributed: the "systemd service wrapper" modules - `core/mqtt_coordinator.py` (~37%), `core/sdr_control.py` (~38%), `core/thermal_monitor.py` (~39%), `trauma/specter_trauma_monitor.py` (~26%), `services/specter_rx_ring_buffer.py` (~29%) - sit well below the ~74-86% the trauma/ward/medical-hub/medical-ai/dashboard modules reach, because their blocking `run()`/`main()` loops and direct hardware interaction (audio I/O, `vcgencmd`, `lsusb`) are inherently harder to unit test and haven't had a dedicated pass yet. A coverage number without knowing *which* modules it's low in is not actionable - this table is the actionable version. Enforcing a `--cov-fail-under` threshold is a deliberate follow-up once those five modules get their own test pass, not before (a threshold set today would either lock in the gap at ~63% or fail CI on every commit until someone does that work).
...
# Web
http://192.168.1.1:5000       Dashboard (operator login required)
http://192.168.1.1:5000/resus RESUS screens (demo, not live - Part 7.2)
http://192.168.1.1:5000/ward  WARD screen (live)
