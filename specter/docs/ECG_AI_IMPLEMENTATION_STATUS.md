...
1. **Acquisition boundary with a disabled Biocare vendor profile** — `medical/ecg_12lead.py` accepts a strict waveform-bearing generic XML subset, requires all 12 canonical leads, explicit units/gain/sample rate/time, rejects ambiguity and poor signals, preserves source evidence, and records every normalization. This is plumbing support, not a claim that the guessed tags match a Biocare export.
...
The public-data/standards pass adds three truthful development paths:

- **WFDB:** pinned `wfdb` support reads physical signals and header calibration, retains exact companion files in a deterministic ZIP with per-file hashes, and records dataset ID/version/license. PTB-XL-style complete 12-lead records can enter the canonical pipeline. MIT-BIH-style two-channel records can be loaded for format/testing work but are explicitly rejected as diagnostic 12-lead inputs.
- **HL7 aECG:** a separate, namespace-aware adapter accepts the HL7 v3 Regulated Studies Annotated ECG Release 1 sequence representation. It requires an explicit time increment, physical origin/scale/unit, timezone-bearing acquisition time, and every canonical lead. It does not imply that Biocare exports this standard.
- **Synthetic fixtures:** public WFDB records can be converted to `SPECTERSyntheticECG` XML for parser and UI regression tests. The root declares `vendorCompatibility="none"` and the document says it is not a Biocare export. These fixtures establish software behavior only.

Explicit source sampling rates from 100 through 8,000 Hz are preserved in the canonical archive. Downsampling/cropping/padding happens only in each model's separately logged preprocessing step. The authenticated `/ecg` screen displays source, device-generated, SPECTER-derived, research-model, agreement, and MedGemma layers separately; it never loads raw waveform arrays into browser memory.
...
- **No PTB-XL, MIT-BIH or other patient dataset is committed.** Dataset licenses, size and patient information require a separate controlled acquisition. The WFDB adapter, manifest validator and split-integrity gate are present, and tests generate only synthetic waveforms locally.
...
1. Export at least three de-identified, waveform-bearing files from the exact iE300, including one normal recording and controlled lead-off/noise cases. Record firmware and export-software versions. Do not rename a SPECTER fixture as a Biocare sample.
...
4. Acquire PTB-XL and approved external corpora under their licenses. Preserve their original WFDB companion files, generate the manifest, and run `validate-dataset` before any evaluation.
